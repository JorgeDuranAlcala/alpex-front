...
const reducer = (state: IDynamicState, action: { type: DynamicActionTypes, payload: any }) => {
  switch (action.type) {
    case DynamicActionTypes.SET_BANK:
      return {
        ...state,
        banks: [...state.banks, {...action.payload, id: Number(state.banks[state.banks.length - 1].id) + 1}]
      }
    break;
...
const bankByIdSelector = (state: IDynamicState, id: string) => state.banks.find(b => b.id == id);
