...
const CommentSection = ({ disable = false }: CommentSectionProps) => {
  ...
  const handleSend = () => {
    for (let i = 0; i < userComments.length; i++) {
      if (userComments[i].id === 1) { // Solo para el usuario con id = 1
              ...
              {user.comments.length > 0 ?
                <div className="comment-view">
                  <div className="user" key={user.id}>
                    <div className="avatar" style={{ backgroundColor: `${user.color}` }}>{user.letter}</div>
                  ...
                  <div className='messages'>
                    {user.comments.map((comment, index) => (
                      <div className="message-globe" key={index}>{comment.message}</div>
                    ))}
